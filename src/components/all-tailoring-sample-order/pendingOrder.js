    ...
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.TailoringSampleOrder.PendingFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response =
            await Requests.TailoringSampleOrder.PendingFilterByFromToDate(
              dateYearFormat(document.getElementById("startDate").value),
              dateYearFormat(document.getElementById("endDate").value),
              page,
              limit
          ...
          setLoading(true);
          const response = await Requests.TailoringSampleOrder.PendingIndex(
            page,
            limit
          );
          if (response && response.data && response.data.body) {
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.TailoringSampleOrder.SearchPendingIndex(
          data
        );
        if (response.data) {
          setData(response.data.body.order);

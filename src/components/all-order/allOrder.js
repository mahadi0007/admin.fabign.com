...
const AllOrder = forwardRef((props, ref) => {
    ...
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response = await Requests.EOrder.AllFilterByFromToDate(
            dateYearFormat(document.getElementById("startDate").value),
            dateYearFormat(document.getElementById("endDate").value),
            page,
            limit
          ...
          const searchResponse = await Requests.EOrder.AllIndex();
          if (
            searchResponse &&
            searchResponse.data &&
            searchResponse.data.body
        ...
        setCurrentPage(page);
        if (props.report) {
          const allDataResponse = await Requests.EOrder.AllIndex();
          if (
            allDataResponse &&
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.EOrder.AllFilterByFromToDateWithSearch(
        dateYearFormat(document.getElementById("startDate").value),
        dateYearFormat(document.getElementById("endDate").value),
        document.getElementById("search").value,
        page,
      ...
      setLoading(true);
      const response = await Requests.EOrder.AllIndex(page, newPerPage);
      setData(response.data.body.order);
      setLimit(newPerPage);
      const searchResponse = await Requests.EOrder.AllIndex();
      if (searchResponse && searchResponse.data && searchResponse.data.body) {
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.EOrder.SearchAllIndex(data);
        if (response.data) {
          setData(response.data.body.order);
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.EOrder.SearchAllIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&

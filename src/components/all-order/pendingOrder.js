...
const PendingOrder = forwardRef((props, ref) => {
    ...
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.EOrder.PendingFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response = await Requests.EOrder.PendingFilterByFromToDate(
            dateYearFormat(document.getElementById("startDate").value),
            dateYearFormat(document.getElementById("endDate").value),
            page,
            limit
          ...
          setLoading(true);
          const response = await Requests.EOrder.PendingIndex(page, limit);
          if (response && response.data && response.data.body) {
            setData(response.data.body.order);
            setTotalRows(response.data.body.total);
            if (props.setDataValue) {
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.EOrder.PendingIndex(page, newPerPage);
      setData(response.data.body.order);
      setLimit(newPerPage);
      setLoading(false);
    }
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.EOrder.SearchPendingIndex(data);
        if (response.data) {
          setData(response.data.body.order);
          if (props.setDataValue) {
            props.setDataValue(
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.EOrder.SearchPendingIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&

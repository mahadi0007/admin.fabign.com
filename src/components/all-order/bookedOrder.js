...
const BookedOrder = forwardRef((props, ref) => {
  ...
  // fetch booked order data
  const fetchBookedEOrder = useCallback(
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.EOrder.BookedFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response = await Requests.EOrder.BookedFilterByFromToDate(
            dateYearFormat(document.getElementById("startDate").value),
            dateYearFormat(document.getElementById("endDate").value),
            page,
            limit
          ...
          setLoading(true);
          const response = await Requests.EOrder.BookedIndex(page, limit);
          if (response && response.data && response.data.body) {
            setData(response.data.body.order);
            setTotalRows(response.data.body.total);
            if (props.setDataValue) {
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.EOrder.BookedFilterByFromToDateWithSearch(
        dateYearFormat(document.getElementById("startDate").value),
        dateYearFormat(document.getElementById("endDate").value),
        document.getElementById("search").value,
        page,
      ...
      setLoading(true);
      const response = await Requests.EOrder.BookedIndex(page, newPerPage);
      setData(response.data.body.order);
      setLimit(newPerPage);
      setLoading(false);
    }
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.EOrder.SearchBookedIndex(data);
        if (response.data) {
          setData(response.data.body.order);
          if (props.setDataValue) {
            props.setDataValue(
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.EOrder.SearchBookedIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&
        ...
        <Card.Simple className="border-0">
          <Card.Body className="p-0">
            ...
            <DataTable
              ...
              clearSearch={() => fetchBookedEOrder(1)}
            />
          </Card.Body>
        </Card.Simple>
      </Container.Column>

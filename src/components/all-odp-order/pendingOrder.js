...
const PendingOrder = forwardRef((props, ref) => {
  ...
  const fetchPendingODPOrder = useCallback(
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.ODPOrder.PendingFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response = await Requests.ODPOrder.PendingFilterByFromToDate(
            dateYearFormat(document.getElementById("startDate").value),
            dateYearFormat(document.getElementById("endDate").value),
            page,
            limit
          ...
          setLoading(true);
          const response = await Requests.ODPOrder.PendingIndex(page, limit);
          if (response && response.data && response.data.body) {
            setData(response.data.body.order);
            setTotalRows(response.data.body.total);
            if (props.setDataValue) {
  ...
  useEffect(() => {
    fetchPendingODPOrder(1);
  }, []);

  // handle paginate page change
  const handlePageChange = (page) => {
    fetchPendingODPOrder(page);
  };
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.ODPOrder.PendingIndex(page, newPerPage);
      setData(response.data.body.order);
      setLimit(newPerPage);
      setLoading(false);
    }
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.ODPOrder.SearchPendingIndex(data);
        if (response.data) {
          setData(response.data.body.order);
          if (props.setDataValue) {
            props.setDataValue(
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.ODPOrder.SearchPendingIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&
        ...
        <Card.Simple className="border-0">
          <Card.Body className="p-0">
            ...
            <DataTable
              ...
              clearSearch={() => fetchPendingODPOrder(1)}
            />
          </Card.Body>
        </Card.Simple>
      </Container.Column>

...
const BookedOrder = forwardRef((props, ref) => {
    ...
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.TailoringOrder.BookedFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response =
            await Requests.TailoringOrder.BookedFilterByFromToDate(
              dateYearFormat(document.getElementById("startDate").value),
              dateYearFormat(document.getElementById("endDate").value),
              page,
              limit
          ...
          setLoading(true);
          const response = await Requests.TailoringOrder.BookedIndex(
            page,
            limit
          );
          if (response && response.data && response.data.body) {
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.TailoringOrder.BookedFilterByFromToDate(
        dateYearFormat(document.getElementById("startDate").value),
        dateYearFormat(document.getElementById("endDate").value),
        page,
        newPerPage
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.TailoringOrder.SearchBookedIndex(data);
        if (response.data) {
          setData(response.data.body.order);
          if (props.setDataValue) {
            props.setDataValue(
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.TailoringOrder.SearchBookedIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&

...
const DeliveryOrder = forwardRef((props, ref) => {
  ...
  const fetchDeliveryEOrder = useCallback(
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.TailoringOrder.DeliveryFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response =
            await Requests.TailoringOrder.DeliveryFilterByFromToDate(
              dateYearFormat(document.getElementById("startDate").value),
              dateYearFormat(document.getElementById("endDate").value),
              page,
              limit
          ...
          setLoading(true);
          const response = await Requests.TailoringOrder.DeliveryIndex(
            page,
            limit
          );
          if (response && response.data && response.data.body) {
  ...
  useEffect(() => {
    fetchDeliveryEOrder(1);
  }, []);

  // handle paginate page change
  const handlePageChange = (page) => {
    fetchDeliveryEOrder(page);
  };
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.TailoringOrder.DeliveryFilterByFromToDate(
        dateYearFormat(document.getElementById("startDate").value),
        dateYearFormat(document.getElementById("endDate").value),
        page,
        newPerPage
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.TailoringOrder.SearchDeliveryIndex(
          data
        );
        if (response.data) {
          setData(response.data.body.order);
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.TailoringOrder.SearchDeliveryIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&
        ...
        <Card.Simple className="border-0">
          <Card.Body className="p-0">
            ...
            <DataTable
              ...
              clearSearch={() => fetchDeliveryEOrder(1)}
            />
          </Card.Body>
        </Card.Simple>
      </Container.Column>

...
const AllOrder = forwardRef((props, ref) => {
    ...
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.TailoringOrder.AllFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response = await Requests.TailoringOrder.AllFilterByFromToDate(
            dateYearFormat(document.getElementById("startDate").value),
            dateYearFormat(document.getElementById("endDate").value),
            page,
            limit
          ...
          const searchResponse =
            await Requests.TailoringOrder.AllFilterByFromToDate(
              dateYearFormat(document.getElementById("startDate").value),
              dateYearFormat(document.getElementById("endDate").value)
            );
          if (
          ...
          setLoading(true);
          const response = await Requests.TailoringOrder.AllIndex(page, limit);
          if (response && response.data && response.data.body) {
            setData(response.data.body.order);
            setTotalRows(response.data.body.total);
            if (props.setDataValue) {
          ...
          const searchResponse = await Requests.TailoringOrder.AllIndex();
          if (
            searchResponse &&
            searchResponse.data &&
            searchResponse.data.body
        ...
        setCurrentPage(page);
        if (props.report) {
          const allDataResponse = await Requests.TailoringOrder.AllIndex();
          if (
            allDataResponse &&
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.TailoringOrder.AllIndex(page, newPerPage);
      setData(response.data.body.order);
      setLimit(newPerPage);
      const searchResponse = await Requests.TailoringOrder.AllIndex();
      if (searchResponse && searchResponse.data && searchResponse.data.body) {
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.TailoringOrder.SearchAllIndex(data);
        if (response.data) {
          setData(response.data.body.order);
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.TailoringOrder.SearchAllIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <Link to={`/dashboard/tailoring-order/show/${row._id}`}>

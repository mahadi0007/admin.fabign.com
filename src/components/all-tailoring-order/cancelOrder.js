...
const CancelOrder = forwardRef((props, ref) => {
    ...
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.TailoringOrder.CancelFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response =
            await Requests.TailoringOrder.CancelFilterByFromToDate(
              dateYearFormat(document.getElementById("startDate").value),
              dateYearFormat(document.getElementById("endDate").value),
              page,
              limit
          ...
          setLoading(true);
          const response = await Requests.TailoringOrder.CancelIndex(
            page,
            limit
          );
          if (response && response.data && response.data.body) {
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.TailoringOrder.CancelFilterByFromToDate(
        dateYearFormat(document.getElementById("startDate").value),
        dateYearFormat(document.getElementById("endDate").value),
        page,
        newPerPage
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.TailoringOrder.SearchCancelIndex(data);
        if (response.data) {
          setData(response.data.body.order);
          if (props.setDataValue) {
            props.setDataValue(
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.TailoringOrder.SearchCancelIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&

...
const PendingOrder = forwardRef((props, ref) => {
  ...
  const fetchPendingEOrder = useCallback(
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.TailoringOrder.PendingFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response =
            await Requests.TailoringOrder.PendingFilterByFromToDate(
              dateYearFormat(document.getElementById("startDate").value),
              dateYearFormat(document.getElementById("endDate").value),
              page,
              limit
          ...
          setLoading(true);
          const response = await Requests.TailoringOrder.PendingIndex(
            page,
            limit
          );
          if (response && response.data && response.data.body) {
  ...
  useEffect(() => {
    fetchPendingEOrder(1);
  }, []);

  // handle paginate page change
  const handlePageChange = (page) => {
    fetchPendingEOrder(page);
  };
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.TailoringOrder.PendingFilterByFromToDate(
        dateYearFormat(document.getElementById("startDate").value),
        dateYearFormat(document.getElementById("endDate").value),
        page,
        newPerPage
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.TailoringOrder.SearchPendingIndex(data);
        if (response.data) {
          setData(response.data.body.order);
          if (props.setDataValue) {
            props.setDataValue(
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.TailoringOrder.SearchPendingIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&
        ...
        <Card.Simple className="border-0">
          <Card.Body className="p-0">
            ...
            <DataTable
              ...
              clearSearch={() => fetchPendingEOrder(1)}
            />
          </Card.Body>
        </Card.Simple>
      </Container.Column>

...
const BookedOrder = forwardRef((props, ref) => {
  ...
  const fetchBookedBulkOrder = useCallback(
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.BulkOrder.BookedFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response = await Requests.BulkOrder.BookedFilterByFromToDate(
            dateYearFormat(document.getElementById("startDate").value),
            dateYearFormat(document.getElementById("endDate").value),
            page,
            limit
          ...
          setLoading(true);
          const response = await Requests.BulkOrder.BookedIndex(page, limit);
          if (response && response.data && response.data.body) {
            setData(response.data.body.order);
            setTotalRows(response.data.body.total);
            if (props.setDataValue) {
  ...
  useEffect(() => {
    fetchBookedBulkOrder(1);
  }, []);

  // handle paginate page change
  const handlePageChange = (page) => {
    fetchBookedBulkOrder(page);
  };
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.BulkOrder.BookedIndex(page, newPerPage);
      setData(response.data.body.order);
      setLimit(newPerPage);
      setLoading(false);
    }
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.BulkOrder.SearchBookedIndex(data);
        if (response.data) {
          setData(response.data.body.order);
          if (props.setDataValue) {
            props.setDataValue(
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.BulkOrder.SearchBookedIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&
        ...
        <Card.Simple className="border-0">
          <Card.Body className="p-0">
            ...
            <DataTable
              ...
              clearSearch={() => fetchBookedBulkOrder(1)}
            />
          </Card.Body>
        </Card.Simple>
      </Container.Column>

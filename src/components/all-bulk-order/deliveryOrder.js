...
const DeliveryOrder = forwardRef((props, ref) => {
  ...
  const fetchDeliveryBulkOrder = useCallback(
    async (page) => {
      try {
        if (
          ...
          setLoading(true);
          const response =
            await Requests.BulkOrder.DeliveryFilterByFromToDateWithSearch(
          ...
          setLoading(true);
          const response = await Requests.BulkOrder.DeliveryFilterByFromToDate(
            dateYearFormat(document.getElementById("startDate").value),
            dateYearFormat(document.getElementById("endDate").value),
            page,
            limit
          ...
          setLoading(true);
          const response = await Requests.BulkOrder.DeliveryIndex(page, limit);
          if (response && response.data && response.data.body) {
            setData(response.data.body.order);
            setTotalRows(response.data.body.total);
            if (props.setDataValue) {
  ...
  useEffect(() => {
    fetchDeliveryBulkOrder(1);
  }, []);

  // handle paginate page change
  const handlePageChange = (page) => {
    fetchDeliveryBulkOrder(page);
  };
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    if (
      ...
      setLoading(true);
      const response = await Requests.BulkOrder.DeliveryIndex(page, newPerPage);
      setData(response.data.body.order);
      setLimit(newPerPage);
      setLoading(false);
    }
  ...
  const handleSearch = async (data) => {
    if (data) {
      if (
        ...
        setsearchLoading(true);
        const response = await Requests.BulkOrder.SearchDeliveryIndex(data);
        if (response.data) {
          setData(response.data.body.order);
          if (props.setDataValue) {
            props.setDataValue(
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.BulkOrder.SearchDeliveryIndex(value);
    if (
      response &&
      response.data.body &&
      response.data.body &&
      ...
      if (document.getElementById("search").value) {
        ...
        if (data) {
          try {
            setLoading(true);
            const response =
              await Requests.BulkOrder.DeliveryFilterByFromToDate(
                dateYearFormat(data),
        ...
        <Card.Simple className="border-0">
          <Card.Body className="p-0">
            ...
            <DataTable
              ...
              clearSearch={() => fetchDeliveryBulkOrder(1)}
            />
          </Card.Body>
        </Card.Simple>
      </Container.Column>

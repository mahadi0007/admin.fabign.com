...
export const Measurement2Form = (props) => {
  const { id } = useParams();
  const {
    control,
    register,
  ...
  const fetchData = useCallback(async () => {
    const response = await Requests.Category2.Index(0, 10);
    ...
    if (props.view || props.update) {
      const response = await Requests.Measurement2.Show(id);
      if (response?.data?.body && response.status === 200) {
        setSizeData(response.data.body);
        setValue("measurement_name", response.data.body.measurement_name);
        setValue("measurementIcon", response.data.body.measurementIcon);
        setValue("category", response.data.body.category._id);
        setValue("measurementVideo", response.data.body.measurementVideo);
        setValue("variable_name", response.data.body.variable_name);
        setValue("size_xs", response.data.body.size_xs);
        setValue("size_s", response.data.body.size_s);
        setValue("size_m", response.data.body.size_m);
        setValue("size_l", response.data.body.size_l);
        setValue("size_xl", response.data.body.size_xl);
        setValue("size_xxl", response.data.body.size_xxl);
      }
      setDefaultCategory({
      ...
      <Container.Row>
        ...
        <Container.Column className="col-lg-6">
          <FormGroup>
            ...
            {(props.view || props.update) && sizeData && defaultCategory && (
              <Controller
                name="category"
                control={control}
                render={({ field }) => (
        ...
        <Container.Column className="col-lg-6">
          <FormGroup>
            {errors.size_xxl && errors.size_xxl.message ? (
              <Text className="text-danger fs-13 mb-0">
                {errors.size_xxl && errors.size_xxl.message}
              </Text>
            ) : (
            ...
            <input
              ...
              defaultValue={sizeData ? sizeData.size_xxl : null}
              {...register("size_xxl", {

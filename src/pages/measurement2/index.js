...
const Index = () => {
  ...
  const fetchMeasurements = useCallback(
    async (page) => {
      try {
        setLoading(true);
        const response = await Requests.Measurement2.Index(page, limit);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <Link to={`/dashboard/measurement2/show/${row._id}`}>
            <SuccessButton type="button" className="btn-circle me-1">
              <Eye size={18} />
            </SuccessButton>
          </Link>
          <Link to={`measurement2/edit/${row._id}`}>
            <SuccessButton type="button" className="btn-circle me-1">
              <Edit2 size={18} />
            </SuccessButton>
          </Link>
  ...
  const handleFabricDelete = async () => {
    try {
      ...
      const response = await Requests.Measurement2.Delete(isDelete.value);
      if (response.status) {
        fetchMeasurements(1);
    ...
    <div>
      <div>
        ...
        <Container.Column className="text-end">
          <Link to={"/dashboard/measurement2/store"}>
            <PrimaryButton className="px-4 mb-3">
              <Text className="fs-15 mb-0">Add Measurement 2</Text>
            </PrimaryButton>
          </Link>
        </Container.Column>

...
const Index = () => {
  ...
  const fetchSubCategory = useCallback(
    async (page) => {
      try {
        setLoading(true);
        const response = await Requests.SubCategory2.Index(page, limit);
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    setLoading(true);
    const response = await Requests.SubCategory2.Index(page, newPerPage);
    setData(response.data.data);
    setLimit(newPerPage);
    setLoading(false);
  };
  ...
  const handleCategoryDelete = async () => {
    try {
      ...
      const response = await Requests.SubCategory2.Delete(
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <Link to={`/dashboard/sub-category2/show/${row._id}`}>
            <SuccessButton type="button" className="btn-circle me-1">
              <Eye size={18} />
            </SuccessButton>
          </Link>

          <Link to={`/dashboard/sub-category2/edit/${row._id}`}>
            <SuccessButton type="button" className="btn-circle me-1">
              <Edit2 size={18} />
            </SuccessButton>
          </Link>
    ...
    <div>
      <div>
        ...
        <Container.Column className="text-end">
          <Link to={"/dashboard/sub-category2/store"}>

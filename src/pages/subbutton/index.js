...
const Index = () => {
    ...
    const fetchButton = useCallback(async (page) => {
        try {
            setLoading(true)
            const response = await Requests.SubButton.Index(page, limit)
    ...
    const handlePerRowsChange = async (newPerPage, page) => {
        setLoading(true)
        const response = await Requests.SubButton.Index(page, newPerPage)
        setData(response.data.data)
        setLimit(newPerPage)
        setLoading(false)
    }
    ...
    //         const response = await Requests.SubButton.MakeDefault(data)
    ...
    const columns = [
        ...
        {
            name: "Sub Category",
            selector: row => row.subcategory ?? "N/A"
        },
        {
            name: "Element",
            selector: row => row.element ?? "N/A"
        },
        {
            name: "Color",
            ...
            cell: row =>
                <div>
                    {row.is_default ? <Text className="fs-14 text-success mb-0 mr-2">Default</Text> :
                        <Text className="fs-14 text-success mb-0 mr-2">Not Default</Text>
                    }
                </div>
        },
        // {

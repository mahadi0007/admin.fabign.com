...
const Edit = () => {
  ...
  const handleFabricUpdate = async (data) => {
    try {
      setLoading(true);
      const response = await Requests.Fabric2.Update(data, id);
  ...
  return (
    <div>
      {/* Title bar */}
      <TitleBar
        mainTitle="Edit Fabric"
        subTitle="Edit your Fabric"
        tag="Home / Fabric /"
        pageTag="Edit Fabric"
      />
      ...
      <Container.Column>
        <Card.Simple className="border-0">
          <Card.Header className="bg-white rounded border-0">
            <div>
              <Text className="mb-0 ps-1 pt-3 fs-17">Edit Fabric</Text>
            </div>
          </Card.Header>
          <hr />

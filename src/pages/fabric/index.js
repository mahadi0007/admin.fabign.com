...
const Index = () => {
    ...
    const [fabricDelete, setFabricDelete] = useState({ show: false, loading: false, value: null })

    // fetch febric data
    const fetchFebric = useCallback(async (page) => {
        try {
            setLoading(true)
            const response = await Requests.Fabric.Index(page, limit)
    ...
    const handleDefault = async (value) => {
        try {
            ...
            const response = await Requests.Fabric.MakeDefault(data)
    ...
    const handleFabricDelete = async () => {
        try {
            ...
            const response = await Requests.Fabric.Delete(fabricDelete.value._id)
            if (response && response.status === 200) {
                fetchFebric(1)
            ...
            cell: row =>
                <div>
                    <DangerButton
                        ...
                        onClick={() => setFabricDelete({ ...fabricDelete, value: row, show: true })}
                    ><Trash2 size={18} />
                    </DangerButton>
                </div>
        }
        ...
        <div>
            <div>
                ...
                <Container.Column className="text-end">
                    <Link to={`/dashboard/fabric/store`}>
            ...
            <DeleteModal
                ...
                message={<Text className="fs-15">Want to delete {fabricDelete.value ? fabricDelete.value.title : null} ?</Text>}
                onHide={() => setFabricDelete({ value: null, show: false, loading: false })}

...
import { FabricTypeForm } from "../../components/form/fabricType";
...
const Index = () => {
  ...
  // fetch febric data
  const fetchFebricType = useCallback(
    async (page) => {
      try {
        setLoading(true);
        const response = await Requests.Types.Index(page, limit);
  ...
  const handlePageChange = (page) => fetchFebricType(page);
  ...
  const handleTypeDelete = async () => {
    try {
      ...
      const response = await Requests.Types.Delete(typeDelete.value._id);
      if (response && response.status === 200) {
        fetchFebricType(1);
  ...
  const handleCreateType = async (data) => {
    ...
    try {
      const response = await Requests.Types.Store(data);
  ...
  const handleUpdateType = async (data) => {
    ...
    try {
      const response = await Requests.Types.Update(data, updateType.value._id);
      if (response.status === 201) {
        Toastify.Success("Fabric Successfully Updated");
        fetchFebricType(1);
      }
  ...
  return (
    <div>
      <div>
        {/* Title bar */}
        <TitleBar
          mainTitle="Manage fabric types"
          subTitle="Manage your fabrics types"
          tag="Home / Fabric /"
        ...
        <Container.Column className="text-end">
          ...
          >
            <Text className="fs-15 mb-0">Add Types</Text>
          </PrimaryButton>
        </Container.Column>
      ...
      >
        <FabricTypeForm handleCreate={handleCreateType} />
      </PrimaryModal>

...
const Index = () => {
  ...
  const fetchData = useCallback(
    async (page) => {
      try {
        setLoading(true);
        const response = await Requests.BacksideElement2.Index(page, limit);
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    setLoading(true);
    const response = await Requests.BacksideElement2.Index(page, newPerPage);
    setData(response.data.data);
    setLimit(newPerPage);
    setLoading(false);
  };
  ...
  const handleDefault = async (value) => {
    ...
    try {
      ...
      const response = await Requests.BacksideElement2.MakeDefault(data);
      if (response && response.status === 201) {
        Toastify.Success(response.data.message);
        fetchData(1);
      }
  ...
  const handleDelete = async (data) => {
    try {
      ...
      const response = await Requests.BacksideElement2.Delete(
  ...
  const columns = [
    ...
    {
      name: "Payout Request Date",
      sortable: true,
      selector: (row) => row.priority,
    },
    {
      name: "Payout Request Amount",
      selector: (row) => (row.category && row.category.title) || "N/A",
      sortable: true,
    },
    {
      name: "Pending Available Balance",
      selector: (row) => (row.category && row.category.title) || "N/A",
      sortable: true,
    },
    {
      ...
      cell: (row) => (
        <div>
          <Link to={`/dashboard/backside-elements2/show/${row._id}`}>
            <SuccessButton type="button" className="btn-circle me-1">
              <Eye size={18} />
            </SuccessButton>
          </Link>
          <Link to={`/dashboard/backside-elements2/edit/${row._id}`}>
            <SuccessButton type="button" className="btn-circle me-1">
              <Edit2 size={18} />
            </SuccessButton>
          </Link>
  ...
  return (
    <div className="mb-4">
      {/* Title bar */}
      <TitleBar
        mainTitle="Manage Balancing"
        subTitle="Manage your balancing"
        tag="Home / Balancing /"
        pageTag="Manage balancing"
      />

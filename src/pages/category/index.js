...
const Index = () => {
  ...
  const fetchCategory = useCallback(
    async (page) => {
      try {
        setLoading(true);
        const response = await Requests.Category.Index(page, limit);
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
    setLoading(true);
    const response = await Requests.Category.Index(page, newPerPage);
    setData(response.data.data);
    setLimit(newPerPage);
    setLoading(false);
  };
  ...
  const handleSearch = async (data) => {
    ...
    const response = await Requests.Search.Category(data);
  ...
  const handleSuggestion = async (value) => {
    ...
    const response = await Requests.Search.Category(value);
  ...
  const handleCategoryDelete = async () => {
    try {
      ...
      const response = await Requests.Category.Delete(categoryDelete.value._id);
      if (response && response.status === 200) {
        fetchCategory(1);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <Link to={`/dashboard/category/show/${row._id}`}>
